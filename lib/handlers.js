...
handlers.users = function(data, callback) {
  ...
  if(acceptableMethod.indexOf(data.method) > -1) {
    ...
    callback(405);      // HTTP method not allowd
  }
}
...
handlers._users.get = function(data, callback) {
  // Check for the phone number is valid
  ...
  if(id && extend) {
    ...
    _data.read('tokens', id, function(err, tokenData) {
      if(!err && tokenData) {
        ...
        callback(400, {'Error': "Specified token dosen't exist"})
      }
    });
  } else {
  ...
  if(id) {
    ...
    _data.read('checks', id, function(err, checkData) {
      if(!err && checkData) {
        // Get the token from the headers
        var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        // Verify that the given token if valid and belongs to the user who created the check
...
// Checks - put
// Required data: id
// Optional data: protocol, url, method, successCodes, timeoutSeconds (one must be same)
              ...
              if(!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if(!err && data) {
                    ...
                    callback(500, {"Error": "Could not find the user who created the check, so could not remove the ckecks from this user object"});
                  }
                });
              } else {
